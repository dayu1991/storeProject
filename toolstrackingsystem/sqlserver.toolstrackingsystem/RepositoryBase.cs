    ...
    public class RepositoryBase<T>:IRepositoryBase<T> where T:class
    {
        ...
        public bool IsExist(PredicateGroup predicateGroup)
        {
            ...
            try
            {
                if (conn == null)
                {
                    conn = CKConnection.GetConnection();
                }
                OpenConnection();
                if (conn.State == ConnectionState.Open)
                {
        ...
        public T GetModel(string strSql, DynamicParameters dynamicParams)
        {
            ...
            try
            {
                ...
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    //var list = conn.Query<T>("select * from db_users where user_name='admin' and pass='123456'");
                    entity = list != null && list.Any() ? list.First<T>() : null;
                }
            }
            catch (Exception)
        ...
        public float QuerySumOrAvgRecordCount(string fullCountSql, DynamicParameters dynamicParams)
        {
            ...
            try
            {
                ...
                if (conn.State == ConnectionState.Open)
                {
                    ...
                    decimal tempCount = (decimal)countObject;
                    long result = (long)tempCount;//装箱后的数据，直接拆箱    
                    resultCount = (float)result;
        ...
        public IEnumerable<T> QueryList(QuerySqlStruct querySqlStruct, out long recordCount, bool limitResultSetMaxCount = true)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (conn.State == ConnectionState.Open)
                    {
                        result = conn.Query<T>(strSql.ToString(), querySqlStruct.DynamicParams);
                    }
                }
                catch (Exception ex)
                {
                ...
                recordCount = QueryRecordCount(strSqlCount.ToString(), querySqlStruct.DynamicParams);
                Close();
            }
            catch (Exception)
            {
