...
{
    public partial class DlgAddCategory : Office2007Form
    {
        ILog logger = log4net.LogManager.GetLogger(typeof(DlgAddCategory));
        ...
        private void Save_Edit_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                bool isExist = _toolInfoService.IsExistCategoryByName(name, 2);
                if (isExist)
                {
            ...
            {
                logger.ErrorFormat("具体位置={0},重要参数Message={1},StackTrace={2},Source={3}", "toolstrackingsystem--DigAddCate--Save", ex.Message, ex.StackTrace, ex.Source);
