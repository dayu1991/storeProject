...
namespace toolstrackingsystem
{
    public partial class FrmlPullRepairedTool : Office2007RibbonForm
    {
            ...
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    if (column is DataGridViewButtonColumn)
                    {
                        ...
                        repairInfo = _toolRepairRecordService.GetToolRepairByToolCodeAndStatus(toolCode, ToolStatusResultEntity.IsCompleted);
                        if (repairInfo != null)
                        {
                            ...
                            repairInfo.ToolStatus = ToolStatusResultEntity.IsPulled;
        ...
        private void superTabStrip1_SelectedTabChanged(object sender, SuperTabStripSelectedTabChangedEventArgs e)
        {
            ...
            {
                logger.ErrorFormat("具体位置={0},重要参数Message={1},StackTrace={2},Source={3}", "FrmlPullRepairedTool--superTabStrip1_SelectedTabChanged", ex.Message, ex.StackTrace, ex.Source);
            }
